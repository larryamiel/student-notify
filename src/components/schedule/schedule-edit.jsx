...
export default function ScheduleEdit ({ schedule, open, toggle, onSuccess }) {
  ...
  const from = schedule.from.toDate();
  const to = schedule.to.toDate();

  // add leading zero to minutes and hours
  const fromHours = from.getHours() < 10 ? `0${from.getHours()}` : from.getHours();
  const fromMinutes = from.getMinutes() < 10 ? `0${from.getMinutes()}` : from.getMinutes();
  const toHours = to.getHours() < 10 ? `0${to.getHours()}` : to.getHours();
  const toMinutes = to.getMinutes() < 10 ? `0${to.getMinutes()}` : to.getMinutes();
        ...
        {(props) => {
          return (
            <form onSubmit={props.handleSubmit} className="mx-auto flex flex-col gap-4">
              <DialogHeader>Edit Schedule</DialogHeader>
